...
export const api = {
  ...
  adminSetup: (input: { email?: string; username?: string; displayName: string; password: string }) => request("/api/admin/setup", { method: "POST", body: JSON.stringify(input) }),
  adminLogin: (loginId: string, password: string) => request("/api/admin/login", { method: "POST", body: JSON.stringify({ loginId, password }) }),
